...
On initial launch or after moving / renaming the `upa/` folder:
...
The GGUF model file is installed as a sibling of the `am` binary by
`amconfig`. If the file is missing, `am flow` exits with a data error and
asks you to reinstall.
...
- Database file: `upa/vin/am.db` (SQLite, WAL mode)
- Error log: `upa/vin/errors.log`
- Corruption marker (transient): `upa/vin/.am_corrupt`

## Viewer

`upa/viewer.html` is a standalone event viewer generated by `amconfig`. Open
it in any browser to inspect your `am.db`.
