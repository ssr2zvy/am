...
HOST_PORT_TTYD=${HOST_PORT_TTYD:-$PORT_TTYD}
HOST_PORT_VIEWER=${HOST_PORT_VIEWER:-$PORT_VIEWER}
...
_ttyd_http_healthy() {
    if command -v busybox >/dev/null 2>&1; then
        busybox wget -q -O /dev/null "http://127.0.0.1:$PORT_TTYD" >/dev/null 2>&1
    elif command -v wget >/dev/null 2>&1; then
        wget -q -O /dev/null "http://127.0.0.1:$PORT_TTYD" >/dev/null 2>&1
    else
        # Fallback to pid-only detection if wget is unavailable.
        return 0
    fi
}
...
is_ttyd_running() {
    if [ -f "$TTYD_PID_FILE" ]; then
        ...
        if [ -n "$pid" ] && [ -d "/proc/$pid" ]; then
            if _ttyd_http_healthy; then
                return 0
            fi
            debug_log "ttyd pid exists but HTTP probe failed; resetting ttyd state"
...
do_vin() {
    echo "am ttydc: http://localhost:$HOST_PORT_TTYD"
    echo "viewer: http://localhost:$HOST_PORT_VIEWER"
}
